...
$name = $email = $mobileno = $password = $filename = $filetmpname = "";
$status = true;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["password"])) {
    ...
    if (strlen($password)<8) {
        $passwordErr = "Password should contain atleast 8 characters. One upper case, One lower case and one number";
        $status = false;
     }
  }
  ...
  if($status){
    $query = "INSERT INTO brokers(name,mobileno,email,image,password) VALUES('$name','$mobileno','$email','$filename','$password')";
